    ...
    switch (action) {
      ...
      case 'placeOrder':
        ...
        if (!productId || !quantity) {
          return { statusCode: 400, body: JSON.stringify({ error: 'Product ID and quantity are required' }) };
        }
        result = await pool.query(
      ...
      case 'adminLogin':
        ...
        if (!adminUsername || !adminPassword) {
          return { statusCode: 400, body: JSON.stringify({ error: 'Admin username and password are required' }) };
        }
        result = await pool.query(
        ...
        if (result.rows.length > 0) {
          return { statusCode: 200, body: JSON.stringify({ message: 'Admin login successful' }) };
        } else {
